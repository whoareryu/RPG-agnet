...
def test_굴에_두고_온_사람이_있으면_섭식_카드가_먼저_온다():
    ...
    h = _hist([("thoma", 40), ("martin", 10)])
    cards = choose_cards(h, ranged_ids={"thoma"}, forsaken=("martin",), slots=3, pool=CARDS)
    assert cards[0][0].key == "devoured"
    assert cards[0][1]["member"] == "martin"


def test_슬롯을_넘지_않는다():
    h = _hist([("thoma", 40), ("martin", 10)])
    assert (
        len(choose_cards(h, ranged_ids={"thoma"}, forsaken=("martin",), slots=1, pool=CARDS)) == 1
    )
