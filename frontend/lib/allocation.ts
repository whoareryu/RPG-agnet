// 능력치 포인트 분배 — 순수 함수. backend/core/rules/stats.py 의 allocate() 와 같은 규칙.
...
export function bump(a: Allocation, key: StatKey, delta: 1 | -1, base: number, freePoints: number): Allocation {
  const cur = a[key] ?? 0;
  const next = cur + delta;
  if (next < 0) return a;
  if (base + next > STAT_MAX) return a;
  if (delta > 0 && total(a) + 1 > freePoints) return a;
  return { ...a, [key]: next };
}

export function validate(a: Allocation, base: number, freePoints: number): string | null {
  for (const k of STAT_KEYS) {
    const v = a[k] ?? 0;
    if (v < 0) return `${k} 에 음수 포인트`;
    if (base + v > STAT_MAX) return `${k} 이 상한 ${STAT_MAX} 을 넘는다`;
  }
